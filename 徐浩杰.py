...
#     args.num_workers = 0
# if args.dynamic_train or args.dynamic_val or args.dynamic_test:
#     raise ValueError("dynamic mode is not supported for now")
    ...
    if args.inference_datasets is None:  # only test on args.dataset
        if args.use_heuristic:
            ...
            train_dataset, train_loader, \
                val_loader, test_loader = get_loaders(args, loggers, run,
                                                      train_dataset, train_loader)
    if args.model not in ['GCN', 'SAGE', 'SEAL', 'GIN']:
        raise ValueError(
            f"Unsupported model type: {args.model}. Supported models are 'GCN', 'SAGE', 'SEAL', and 'GIN'.")
    if args.foundation_mode:
        ...
        if args.load_model:  # 如果有预训练模型路径，则加载模型
            ...
            if save_model_path is None:
                raise ValueError(
                    "save_model_path is not defined. Please ensure the model is saved correctly during pretraining.")
        ...
        if not train_flg:
            if args.inference_datasets is None:
                ...
                for one_dataset_name in args.inference_datasets:
                    train_dataset, train_loader, \
                        val_loader, test_loader = get_loaders(args, loggers, run,
                                                              train_dataset, train_loader, one_dataset_name)
                    test_print(cache_support=True)
                    # test_print(cache_support=False);exit(0)
            break
        loss = train(epoch)
